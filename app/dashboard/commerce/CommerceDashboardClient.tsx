...
function toFormInput(data: CommerceBriefFormData): FormInput {
  return {
    course: data.subject,
    week: data.week,
    difficulty: data.difficulty,
    projects: data.priorWork,
    language: data.assignmentType,
    syllabus: data.syllabus,
  };
}

function formatCommerceBrief(brief: BriefSection) {
  return [
    "## Case Study Problem",
    brief.problem,
    "",
...
export default function CommerceDashboardClient({
  ...
  const [generationInput, setGenerationInput] = useState<CommerceBriefFormData | null>(null);
  ...
  const handleGenerate = useCallback(async (data: CommerceBriefFormData, retryCount = 0) => {
    ...
    try {
      ...
      setBrief(parsed);
      setStreamingText(formatCommerceBrief(parsed));
  ...
  const handlePushback = useCallback(async (pushbackText: string, retryCount = 0) => {
    ...
    const previousBrief = formatCommerceBrief(brief);
    ...
    try {
      ...
      setStreamingText(formatCommerceBrief(currentBriefSnapshot));
    } finally {
      setIsStreaming(false);
      setIsRefining(false);
    }
        ...
        <header className="sticky top-0 z-40 flex items-center justify-between border-b border-[var(--night-line)] bg-[rgba(10,10,10,0.82)] px-3 py-3 backdrop-blur-xl sm:px-6 md:px-10 sm:py-4">
          <div className="flex items-center gap-2 sm:gap-6">
            <Link href="/" className="flex shrink-0 items-center gap-2">
              ...
              <span className="hidden text-xs font-medium text-[var(--night-glow)] sm:block">/ Commerce studio</span>
            </Link>
                  ...
                  <div className="mx-auto flex w-full max-w-2xl flex-col items-center text-center">
                    <div className="mb-6 flex flex-col items-center px-1 sm:mb-10">
                      ...
                      <p className="mt-4 max-w-2xl text-sm leading-7 text-[var(--text-dim)] sm:text-base">
                        Tell Nextstep what you&apos;re studying. We&apos;ll shape it into one commerce brief with sharper assumptions, clearer constraints, and a real decision to defend.
                      </p>
                    </div>
                ...
                {genView === "brief" && (
                  <div className="flex w-full flex-col">
                    ...
                    {streamingText && !isStreaming && (
                      ...
                      >
                        <strong className="text-[var(--night-glow)]">Next step:</strong> pressure-test the assumptions, run the numbers, and decide what you would actually recommend.
                      </motion.div>
                    )}
                  </div>
                )}
            ...
            {tab === "history" && (
              ...
              >
                <div className="mb-6 flex flex-col justify-between gap-3 sm:mb-8 sm:flex-row sm:items-center">
                  <div>
                    <h2 className="display-font text-3xl text-white">Your Commerce Briefs</h2>
                ...
                {briefs.length === 0 ? (
                  <div className="rounded-[2rem] border border-white/[0.08] bg-white/[0.02] px-4 py-16 text-center">
                    ...
                    <p className="mb-6 text-white/50">No commerce briefs generated yet.</p>
                    <button
