...
import CommerceBriefForm, { CommerceBriefFormData } from "@/components/commerce/CommerceBriefForm";
import { CommerceBriefDisplay } from "@/components/commerce/CommerceBriefDisplay";
...
export default function CommerceDashboardClient({ initialBriefs, user }: { initialBriefs: SavedBrief[]; user: User }) {
  ...
  const handleGenerate = useCallback(async (data: CommerceBriefFormData) => {
    ...
    try {
      const response = await fetch("/api/generate-brief", {
        ...
        body: JSON.stringify({ domain: "commerce", ...data }),
      });
      if (!response.ok) {
      ...
      fetch("/api/briefs", {
        ...
        body: JSON.stringify({
          domain: "commerce",
          formInput: { course: data.subject, week: data.unit, projects: data.priorWork, language: data.tool },
  ...
  if (authLoading) {
    return (
      <div className="min-h-screen bg-[#080E0A] text-white flex items-center justify-center">
  ...
  return (
    <div className="min-h-screen bg-[#080E0A] text-white flex flex-col">
      {/* Header */}
      <header className="border-b border-emerald-500/[0.08] px-3 sm:px-6 md:px-10 py-3 sm:py-4 flex items-center justify-between bg-[#080E0A]/90 backdrop-blur-md sticky top-0 z-40">
        <div className="flex items-center gap-2 sm:gap-6">
          <Link href="/" className="flex items-center gap-2 shrink-0">
            ...
            <span className="text-emerald-400 text-xs font-medium hidden sm:block">/ Commerce</span>
          </Link>
          <nav className="flex items-center gap-0.5 sm:gap-1">
            {(["generate", "history"] as const).map(t => (
              <button key={t} onClick={() => setTab(t)}
                className={`px-3 sm:px-4 py-1.5 rounded-full text-xs sm:text-sm font-medium transition-all capitalize ${tab === t ? "bg-emerald-500/15 text-emerald-400" : "text-white/50 hover:text-white"}`}>
                {t === "generate" ? "Generate" : `My Briefs ${briefs.length > 0 ? `(${briefs.length})` : ""}`}
              </button>
            ))}
          </nav>
        </div>
                ...
                <div className="w-full max-w-2xl mx-auto flex flex-col items-center text-center">
                  <div className="mb-6 sm:mb-10 flex flex-col items-center px-1">
                    ...
                    <h1 className="text-3xl sm:text-4xl md:text-5xl font-bold text-white leading-tight" style={{ fontFamily: "Georgia, serif" }}>
                      What are you{" "}
                      <span className="text-emerald-400 italic">analysing today?</span>
                    </h1>
                    <p className="mt-3 text-white/45 text-sm sm:text-base max-w-xl leading-relaxed">
                      Tell Candela what you&apos;re studying. We&apos;ll generate a case study grounded in real companies and market data.
                    </p>
                  </div>
                  {genError && (
                  ...
                  <div className="w-full text-left">
                    <CommerceBriefForm onSubmit={handleGenerate} isSubmitting={isStreaming} />
                  </div>
                </div>
              )}
              ...
              {genView === "brief" && (
                <div className="flex flex-col w-full">
                  <button onClick={handleReset}
                    ...
                    <span className="group-hover:-translate-x-0.5 transition-transform">←</span> New case study
                  </button>
                  {currentSubject && (
                    <div className="mb-4 flex flex-wrap gap-2">
                      <span className="px-3 py-1 bg-emerald-500/10 border border-emerald-500/20 rounded-full text-xs text-emerald-400">📊 {currentSubject}</span>
                    </div>
                  )}
                  <CommerceBriefDisplay rawText={streamingText} isStreaming={isStreaming} courseName={currentSubject} />
                  {streamingText && !isStreaming && (
                    <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.4 }}
                      className="mt-6 p-4 bg-emerald-500/5 border border-emerald-500/15 rounded-xl text-emerald-400/70 text-sm">
                      💡 <strong className="text-emerald-300">Next step:</strong> Work through the analysis checkpoints, then attempt the advanced challenge for your portfolio.
                    </motion.div>
                  )}
                </div>
              )}
          ...
          {tab === "history" && (
            <motion.div key="history" initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -8 }} transition={{ duration: 0.3 }}>
              <div className="mb-6 sm:mb-8 flex flex-col sm:flex-row sm:items-center justify-between gap-3">
                <div>
                  <h2 className="text-2xl sm:text-3xl font-bold text-white" style={{ fontFamily: "Georgia, serif" }}>Your Case Studies</h2>
                  <p className="text-white/40 text-sm mt-1">{briefs.length === 0 ? "No case studies yet." : `${briefs.length} generated`}</p>
                </div>
                <button onClick={() => { setTab("generate"); handleReset(); }}
                  className="self-start sm:self-auto px-4 sm:px-5 py-2 sm:py-2.5 bg-emerald-600 hover:bg-emerald-500 text-white text-sm font-medium rounded-full transition-colors">
                  + New case study
                </button>
              </div>
              {briefs.length === 0 ? (
                <div className="text-center py-16 border border-emerald-500/10 rounded-2xl bg-emerald-500/[0.02]">
                  <div className="text-4xl mb-4">📊</div>
                  <p className="text-white/50 mb-6">No case studies generated yet.</p>
                  <button onClick={() => setTab("generate")}
                    className="inline-flex items-center px-6 py-3 bg-emerald-600 hover:bg-emerald-500 text-white font-medium rounded-full text-sm transition-colors">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3 sm:gap-4">
                  {briefs.map((b, i) => (
                    <motion.div key={b._id} initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.06 }}
                      className="bg-emerald-500/[0.03] border border-emerald-500/[0.1] rounded-2xl p-5 flex flex-col hover:border-emerald-500/30 transition-colors">
