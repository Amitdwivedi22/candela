...
export default function EngineeringDashboardClient({ initialBriefs, user }: { initialBriefs: SavedBrief[]; user: User }) {
  ...
  const handleGenerate = useCallback(async (data: EngineeringBriefFormData) => {
    ...
    try {
      ...
      fetch("/api/briefs", {
        ...
        body: JSON.stringify({
          domain: "engineering",
          formInput: { course: data.subject, week: data.semester, projects: data.priorWork, language: data.software },
  ...
  if (authLoading) {
    return (
      <div className="min-h-screen bg-[#0A0704] text-white flex items-center justify-center">
  ...
  return (
    <div className="min-h-screen bg-[#0A0704] text-white flex flex-col">
      {/* Header */}
      <header className="border-b border-amber-500/[0.08] px-3 sm:px-6 md:px-10 py-3 sm:py-4 flex items-center justify-between bg-[#0A0704]/90 backdrop-blur-md sticky top-0 z-40">
        <div className="flex items-center gap-2 sm:gap-6">
          <Link href="/" className="flex items-center gap-2 shrink-0">
            ...
            <span className="text-amber-400 text-xs font-medium hidden sm:block">/ Engineering</span>
          </Link>
          <nav className="flex items-center gap-0.5 sm:gap-1">
            {(["generate", "history"] as const).map(t => (
              <button key={t} onClick={() => setTab(t)}
                className={`px-3 sm:px-4 py-1.5 rounded-full text-xs sm:text-sm font-medium transition-all capitalize ${tab === t ? "bg-amber-500/15 text-amber-400" : "text-white/50 hover:text-white"}`}>
                ...
                <div className="w-full max-w-2xl mx-auto flex flex-col items-center text-center">
                  <div className="mb-6 sm:mb-10 flex flex-col items-center px-1">
                    ...
                    <h1 className="text-3xl sm:text-4xl md:text-5xl font-bold text-white leading-tight" style={{ fontFamily: "Georgia, serif" }}>
                      What are you{" "}
                      <span className="text-amber-400 italic">designing today?</span>
                    </h1>
                    <p className="mt-3 text-white/45 text-sm sm:text-base max-w-xl leading-relaxed">
                      Tell Candela what you&apos;re studying. We&apos;ll generate an engineering problem grounded in real physics and codes.
                    </p>
                  </div>
                  {genError && (
              ...
              {genView === "brief" && (
                <div className="flex flex-col w-full">
                  <button onClick={handleReset}
                    ...
                    <span className="group-hover:-translate-x-0.5 transition-transform">←</span> New design problem
                  </button>
                  {currentSubject && (
                    <div className="mb-4 flex flex-wrap gap-2">
                      <span className="px-3 py-1 bg-amber-500/10 border border-amber-500/20 rounded-full text-xs text-amber-400">⚙️ {currentSubject}</span>
                    </div>
                  )}
                  <EngineeringBriefDisplay rawText={streamingText} isStreaming={isStreaming} courseName={currentSubject} />
                  {streamingText && !isStreaming && (
                    <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.4 }}
                      className="mt-6 p-4 bg-amber-500/5 border border-amber-500/15 rounded-xl text-amber-400/70 text-sm">
                      💡 <strong className="text-amber-300">Next step:</strong> Start with the calculation scaffold and work through the analysis checkpoints.
                    </motion.div>
                  )}
                </div>
              )}
          ...
          {tab === "history" && (
            <motion.div key="history" initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -8 }} transition={{ duration: 0.3 }}>
              <div className="mb-6 sm:mb-8 flex flex-col sm:flex-row sm:items-center justify-between gap-3">
                <div>
                  <h2 className="text-2xl sm:text-3xl font-bold text-white" style={{ fontFamily: "Georgia, serif" }}>Your Engineering Briefs</h2>
                  <p className="text-white/40 text-sm mt-1">{briefs.length === 0 ? "No briefs yet." : `${briefs.length} generated`}</p>
                </div>
                <button onClick={() => { setTab("generate"); handleReset(); }}
                  className="self-start sm:self-auto px-4 sm:px-5 py-2 sm:py-2.5 bg-amber-600 hover:bg-amber-500 text-white text-sm font-medium rounded-full transition-colors">
                  + New brief
                </button>
              </div>
              {briefs.length === 0 ? (
                <div className="text-center py-16 border border-amber-500/10 rounded-2xl bg-amber-500/[0.02]">
                  <div className="text-4xl mb-4">⚙️</div>
                  <p className="text-white/50 mb-6">No engineering problems generated yet.</p>
                  <button onClick={() => setTab("generate")}
                    className="inline-flex items-center px-6 py-3 bg-amber-600 hover:bg-amber-500 text-white font-medium rounded-full text-sm transition-colors">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3 sm:gap-4">
                  {briefs.map((b, i) => (
                    <motion.div key={b._id} initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.06 }}
                      className="bg-amber-500/[0.03] border border-amber-500/[0.1] rounded-2xl p-5 flex flex-col hover:border-amber-500/30 transition-colors">
